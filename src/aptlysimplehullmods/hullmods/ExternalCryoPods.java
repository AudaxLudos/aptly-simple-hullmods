...
public class ExternalCryoPods extends BaseLogisticsHullMod {
    ...
    public void addPostDescriptionSection(TooltipMakerAPI tooltip, HullSize hullSize, ShipAPI ship, float width, boolean isForModSpec) {
        ...
        tooltip.addPara("Increases the crew capacity by %s based on hull size", opad, good, Math.round(CREW_MODIFIER.get(HullSize.FRIGATE).intValue()) + "/"
                + Math.round(CREW_MODIFIER.get(HullSize.DESTROYER).intValue()) + "/"
                + Math.round(CREW_MODIFIER.get(HullSize.CRUISER).intValue()) + "/"
                + Math.round(CREW_MODIFIER.get(HullSize.CAPITAL_SHIP).intValue()) + " points");
