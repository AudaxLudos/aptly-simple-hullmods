...
public class ExternalFuelTanks extends BaseLogisticsHullMod {
    ...
    public void addPostDescriptionSection(TooltipMakerAPI tooltip, HullSize hullSize, ShipAPI ship, float width, boolean isForModSpec) {
        ...
        tooltip.addPara("Increases the fuel capacity by %s based on hull size", opad, good, Math.round(FUEL_MODIFIER.get(HullSize.FRIGATE).intValue()) + "/"
                + Math.round(FUEL_MODIFIER.get(HullSize.DESTROYER).intValue()) + "/"
                + Math.round(FUEL_MODIFIER.get(HullSize.CRUISER).intValue()) + "/"
                + Math.round(FUEL_MODIFIER.get(HullSize.CAPITAL_SHIP).intValue()) + " points");
